...
A_n = rpLibGetDouble(io,'input.phase(mat).string(narea).current');
A_p = rpLibGetDouble(io,'input.phase(mat).string(parea).current');
%L_array= [0.1:0.1:0.9, 1:1:20];     % thickness array to test [mm]
L_array = rpLibGetDouble(io,'input.phase(mat).string(thick).current');
...
R_L_array = rpLibGetDouble(io,'input.phase(mat).string(R_L).current');
LOAD_MATCH = rpLibGetDouble(io,'input.phase(mat).choice(Match).current');
...
Psi_h = rpLibGetDouble(io,'input.phase(mat).string(coeff).current');
Psi_c = rpLibGetDouble(io,'input.phase(mat).string(sink).current');
...
switch choice
    ...
    case 4
        % FILL FACTOR
        min = rpLibGetDouble(io,'input.phase(mat).group(horizontal).group(ind).group(Fill_Fact).number(min).current');
        step = rpLibGetDouble(io,'input.phase(mat).group(horizontal).group(ind).group(Fill_Fact).number(step).current');
        max = rpLibGetDouble(io,'input.phase(mat).group(horizontal).group(ind).group(Fill_Fact).number(max).current');
        Fill_Factor_array = min: step: max;
		len = length(Fill_Factor_array);
end
...
for index_FillFactor=1:length(FillFactor_array)
    ...
    for index_A=1:length(A_array),
        if choice == 2
        A_n=A_array(index_A);                   % Area of n-type element
        A_p=A_ratio*A_array(index_A);           % Area of p-type element
        end
...
switch choice
    case 1
        ...
        rpLibPutString(io,'output.curve(Eff_system).xaxis.label','Thickness (mm)',0);
        rpLibPutString(io,'output.curve(Eff_TE).xaxis.label','Thickness (mm)',0);
    ...
    case 2
        ...
        rpLibPutString(io,'output.curve(Eff_system).xaxis.label','Total Area (mm^2)',0);
        rpLibPutString(io,'output.curve(Eff_TE).xaxis.label','Total Area (mm^2)',0);
    ...
    case 3
        ...
        rpLibPutString(io,'output.curve(Eff_system).xaxis.label','Load Resistance (Ohm)',0);
        rpLibPutString(io,'output.curve(Eff_TE).xaxis.label','Load Resistance (Ohm)',0);
    ...
    case 4
        mat = Fill_Factor_array;
        rpLibPutString(io,'output.field(ncurve).about.xaxis.label','Distance from Top Surface (mm)',0);
        rpLibPutString(io,'output.field(ncurve).about.yaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.field(pcurve).about.xaxis.label','Distance from Top Surface (mm)',0);
        rpLibPutString(io,'output.field(pcurve).about.yaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.curve(P_out).xaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.curve(Eff_system).xaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.curve(Eff_TE).xaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.curve(Cur).xaxis.label','Fill Factor ',0);
        rpLibPutString(io,'output.curve(V_out).xaxis.label','Fill Factor ',0);
end
%% Output Plots
...
xydata = [mat;DATA(:,21)'];    
str = sprintf('%12g %12g\n', xydata);
rpLibPutString(io,'output.curve(Eff_system).component.xy',str,0);

xydata = [mat;DATA(:,19)'];
str = sprintf('%12g %12g\n', xydata);
rpLibPutString(io,'output.curve(Eff_TE).component.xy',str,0);
