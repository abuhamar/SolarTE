...
% - Kevin & Je-Hyeong (updated Summer 2017)

clear all;
format long;
TEMP_DEPENDENT=1; TEMP_DEPENDENT_SIM=2; CONSTANT=3;
YES=1;NO=0;

% DATA SELECTION
MODE= TEMP_DEPENDENT_SIM;
LOAD_MATCH=YES;         % Load matching condition (R_L = R_internal)

%% Material properties database files
  
if (MODE==TEMP_DEPENDENT)
     DB_n = csvread('DB_BiTeSe_n.csv',1,0);
     DB_p = csvread('DB_BiSbTe_p.csv',1,0);
elseif (MODE==TEMP_DEPENDENT_SIM)
     DB_n = csvread('DB_BiTeSe_n_sim.csv',1,0);
     DB_p = csvread('DB_BiSbTe_p_sim.csv',1,0);
elseif (MODE==CONSTANT)
    DB_n = csvread('DB_constant_n.csv',1,0);
    DB_p = csvread('DB_constant_p.csv',1,0);
end    

N = 10;             % number of simulation segments of each leg 

%% User input parameters ================
G = 700;            % Solar energy input from one sun [W/m2]
%Conct_array = [1 10:10:100];         % solar concentration [suns]
Conct_array = [60];
%Tau_glass = 1;   % transmittance through top glass 
Tau_glass = 0.94;   % transmittance through top glass 
Alpha_abs = 0.9;   % Absorptance of the solar absorber
Emissivity_abs = 0.15;      % Emissivity of the absorber
...
Area_abs = 100*100;       % Absorber area mm^2     
Total_Area = Area_abs;     % Total TE module area mm^2 

%FillFactor_array = [0.1:0.1:0.9];
FillFactor_array = 0.2;
        ...
        for index_L=1:length(L_array),
            ...
            for index_psih=1:length(Psi_h_array)
                for index_psic=1: length(Psi_c_array)
                    ...
                    DATA_Pout(index_L,index_FillFactor+1)=P_out;
